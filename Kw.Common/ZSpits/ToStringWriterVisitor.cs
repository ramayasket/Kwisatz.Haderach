    ...
    public class ToStringWriterVisitor : BuiltinsWriterVisitor {
        ...
        protected override void WriteBinary(BinaryExpression expr) {
            ...
            var op = expr.NodeType switch
            {
                ...
                GreaterThanOrEqual => ">=",
                LessThanOrEqual => "<=",
                var x when x.Inside(Add, AddChecked) => "+",
                var x when x.Inside(AddAssign, AddAssignChecked) => "+=",
                var x when x.Inside(Subtract, SubtractChecked) => "-",
                var x when x.Inside(SubtractAssign, SubtractAssignChecked) => "-=",
                Divide => "/",
                DivideAssign => "/=",
                Modulo => "%",
                ModuloAssign => "%=",
                var x when x.Inside(Multiply, MultiplyChecked) => "*",
                var x when x.Inside(MultiplyAssign, MultiplyAssignChecked) => "*=",
                LeftShift => "<<",
                LeftShiftAssign => "<<=",
                RightShift => ">>",
                RightShiftAssign => ">>=",
        ...
        protected override void WriteUnary(UnaryExpression expr) {
            Write(
                expr.NodeType switch
                {
                    var x when x.Inside(Negate, NegateChecked) => "-",
                    Not => "Not(",
                    IsFalse => "IsFalse(",
                    IsTrue => "IsTrue(",
                    OnesComplement => "~(",
            ...
            switch (expr.NodeType) {
                case var x when x.Inside(Negate, NegateChecked, UnaryPlus, PreDecrementAssign, PreIncrementAssign, Quote):
                    break;
                case TypeAs:
                    Write(" As ");
                    Write(expr.Type.Name);
                    Write(')');
                    break;
                case var x when x.Inside(ExpressionType.Convert, ConvertChecked):
