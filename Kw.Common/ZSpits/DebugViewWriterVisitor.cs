    ...
    public class DebugViewWriterVisitor : BuiltinsWriterVisitor {
        ...
        private static int GetOperatorPrecedence(Expression node) =>
            node.NodeType switch
            {
                ...
                ExclusiveOr => 5,
                And => 6,
                var x when x.Inside(Equal, NotEqual) => 7,
                ...
                var x when x.Inside(LeftShift, RightShift) => 9,
                var x when x.Inside(Add, AddChecked, Subtract, SubtractChecked) => 10,
                var x when x.Inside(Divide, Modulo, Multiply, MultiplyChecked) => 11,
                ...
                var x when x.Inside(Constant, Parameter) => 15,

                var x when x.Inside(PostIncrementAssign, PostDecrementAssign) => 14,
                _ => 14
            };
